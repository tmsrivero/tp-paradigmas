...
presupuestoRemanente := Prompter prompt: 'Ingrese el presupuesto remanente: ' asNumber.
switch := true.
[switch] whileTrue: [
	nom := Prompter prompt: 'Ingrese el nombre del empleado a buscar: '.
	ape := Prompter prompt: 'Ingrese el apellido del empleado a buscar: '.
	emp := empleados detect: [ :x | (x nombre = nom) and: [x apellido = ape]] ifNone:[nil].
	emp notNil ifTrue: [
		(emp class = 'Actor') ifTrue: [emp cargaCachet].
		(emp class = 'EquipoDireccion') ifTrue: [emp cargaPorcentaje].
		(emp class = 'Permanente') ifTrue: [emp cargaPlus].]
		
].
"Buscar empleados por nombre y apellido en colleción empleados, asignar cache, plus o porcentaje al añadirlo" 
...
x := Prompter prompt: 'Ingrese el porcentaje sobre el presupuesto asignado[0-100]: ' asNumber.
[((x > 100) or: [x <= 0]) or: [x]] whileTrue: [x := Prompter prompt: 'Ingrese el porcentaje sobre el presupuesto asignado[0-100]: ' asNumber.].
porcentaje := x/100.! !
